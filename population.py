...
class PopulationOneLayer:
    def __init__(self, load=False, path=None, gen=0, res_file="./result/result.txt"):
        ...
        if load:
            ...
            self.best_snake.nn.weights_ih, self.best_snake.nn.weights_ho = self.load_best_snake_from_gen(gen,path)
        else:
            self.best_snake = None
        self.best_score = 0.0
        self.total_best_score = 0.0
    ...
    def create_from_saved(self, size):
        ...
        for i in range(len(self.snakes)):
                self.snakes[i].reset()
                self.snakes[i].nn.weights_ih, self.snakes[i].nn.weights_ho = self.best_snake.nn.weights_ih, self.best_snake.nn.weights_ho
                if i != 0: self.snakes[i].nn.mutate()
        print('Population copied')
