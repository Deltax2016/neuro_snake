...
# snake class
snake_len = 3
snake_list = [[120, 200], [140, 200], [160, 200]]
food_list = []
...
def save_data(weights_input, bias_input, weights_hidden1, bias_hidden1, weights_hidden2, bias_hidden2, weights_output, bias_output):
    np.save("/saved/weights_input.npy", weights_input)
    np.save("/saved/bias_input.npy", bias_input)
    np.save("/saved/weights_hidden1.npy", weights_hidden1)
    np.save("/saved/bias_hidden1.npy", bias_hidden1)
    np.save("/saved/weights_hidden2.npy", weights_hidden2)
    np.save("/saved/bias_hidden2.npy", bias_hidden2)
    np.save("/saved/weights_output.npy", weights_output)
    np.save("/saved/bias_output.npy", bias_output)
...
while running:
    ...
    if keys[pygame.K_LEFT]:
        if direction != 1:
            ...
            pred = nn.train(X, np.array([[1,0,0,0]]))
    if keys[pygame.K_RIGHT]:
        if direction != 0:
            direction = 1
            snake_list.append([snake_list[-1][0] + 20, snake_list[-1][1]])
            pred = nn.train(X, np.array([[0,1,0,0]]))
    if keys[pygame.K_UP]:
        if direction != 3:
            direction = 2
            snake_list.append([snake_list[-1][0], snake_list[-1][1] - 20])
            pred = nn.train(X, np.array([[0,0,1,0]]))
    if keys[pygame.K_DOWN]:
        if direction != 2:
            direction = 3
            snake_list.append([snake_list[-1][0], snake_list[-1][1] + 20])
            pred = nn.train(X, np.array([[0,0,0,1]]))
    if keys[pygame.K_SPACE]:
        save_data(nn.weights_input, nn.bias_input, nn.weights_hidden1, nn.bias_hidden1, nn.weights_hidden2, nn.bias_hidden2, nn.weights_output, nn.bias_output)
        running = False
